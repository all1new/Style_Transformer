...
def train(args):
    ...
    transformer = TransformerNet().to(device)   # 定义风格转换模型
    optimizer = Adam(transformer.parameters(), args.lr)     # 优化器
    mse_loss = nn.MSELoss()     # 均方差损失函数

    vgg = vgg16(requires_grad=False).to(device) # 特征提取网络
    ...
    # 先对style数据归一化,然后进行特征提取
    features_style = vgg(utils.normalize_batch(style))  
    # 获得style数据特征的格拉姆矩阵
    gram_style = [utils.gram_matrix(y) for y in features_style]

    # 开始训练数据
    for e in range(args.epochs):    # 训练周期
        transformer.train()
        agg_content_loss = 0.
        agg_style_loss = 0.
        count = 0
        for batch_id, (x, _) in enumerate(train_loader):    # 数据集统计
            n_batch = len(x)
            count += n_batch
            optimizer.zero_grad()
